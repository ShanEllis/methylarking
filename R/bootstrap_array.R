...
#'
#' @param CpG DNAm data from array \code{CpG}
...
#' 
#' @examples
#' bootstrap_array(CpG, covariates=covariates, variance=0.75,JOBID=2)

bootstrap_array<-function(CpG,covariates,variance,JOBID){
	coef=2
	B=1
	require(limma)
	require(methylKit)
...
bootIndexes<-replicate(B, sample(1:ncol(CpG),replace=TRUE),simplify=TRUE)

#fit the alternative model
message('fiting the alternative model')
	DMRfit_log10cov <- lmFit(CpG,design=model.matrix(~treatment +as.matrix(covariates)))
...
message('fiting the null model')
	...
	DMRfit_null <- lmFit(CpG,design=model.matrix(~as.matrix(covariates)))
...
message('refit full model with pseudonull')
	DMRfit_pseudonull <- lmFit(pseudonull,design=model.matrix(~treatment +as.matrix(covariates)))
...
message('saving output')
	save(pseudo,bootIndexes, differ ,file=paste("Bootstrap",JOBID,".rda",sep=""))
}